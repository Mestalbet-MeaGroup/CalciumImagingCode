...
Betn2 = betChoices(:,2);
clear ii;
h = waitbarwithtime(0,'Calculating KL...');
for ii=1:size(DataSet,1)
    ...
    for j=1:numel(Betn1)
        [~,~,bw1,ibi1]=UnsupervisedBurstDetection9Well(DataSet{ii}.tGcamp{Betn1(j)},DataSet{ii}.icGcamp{Betn1(j)});
        [~,~,bw2,ibi2]=UnsupervisedBurstDetection9Well(DataSet{ii}.tBase{Betn2(j)},DataSet{ii}.icBase{Betn2(j)});
        ...
        DataSet{ii}.klIBI(j) = kldiv(DataSet{ii}.uIBI,probIBI1+eps,probIBI2+eps ,'sym');
        DataSet{ii}.klBW(j) = kldiv(DataSet{ii}.uBW,probBW1+eps,probBW2+eps ,'sym');
    end
%     waitbarwithtime(ii/size(DataSet,1),h);
end