...
% figure,
% for nn=1:N,
%     subplot(ceil(sqrt(N)),ceil(sqrt(N)),nn);
%     nb = EAfile.NERO.NERO_CHANNELMAT(nn,:);
%     [id,rankid] = ismember(EAfile.RAWDATA.CHANNELMAP,nb);
%     FSRO = reshape(rankid,size(EAfile.RAWDATA.CHANNELMAP));
%     imagesc(FSRO); colormap([[1 1 1];flipud(jet(sum(~isnan(nb))))]);
% %     imagesc_RGB(FSRO,flipud(jet(sum(~isnan(nb)))));
% end
% 
