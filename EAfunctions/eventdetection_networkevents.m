function [NETWORKEVENTID,NETWORKEVENTONSETS,NETWORKEVENTOFFSETS]=eventdetection_networkevents(spiketime,eventtime,eventid,varargin)
%%
%% default settings ===============================
%--------------------------------------------------
% Onset window
...
[te, idfirst] = unique(eventid,'first');
temp(idfirst)=1;
temp  = cumsum(temp);
...
temp = zeros(size(eventid));
[te, idlast] = unique(eventid,'last');
temp(idlast)=1;
temp = cumsum(temp);
temp(idlast)=temp(idlast)-1;
eventid_offset = temp; %+offset_Nevents-1;

% number of events lasting at the moment of a spike
events_lasting = eventid_onset-eventid_offset;
